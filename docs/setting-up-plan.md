...
- [ ] **T4.9 Songs 翻页**（Phase 4 收尾评审新增，用户裁定 2026-09-11「Phase 5 前补」）：Songs 页页脚最小翻页控件接 `libraryStore.setPage`（offset/limit 机制已就绪，页长 50、上界 200）；上一页/下一页 + 页码呈现；**禁假总数**——总数通道缺失，用「第 N 页」形态而非「共 M 条」，末页判定=返回行数 < limit 时禁用下一页、首页禁用上一页；i18n 键追加（zh-CN.json）；单测：控件渲染态（首/中/末页禁用逻辑）+ setPage 驱动取数参数断言；e2e 不要求（fixtures 7 首单页）。
