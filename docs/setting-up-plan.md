...
- [x] **T5.3 AudioEngine + playbackService + playerStore**：AudioEngine 挂 `<audio>`，`crossOrigin` 不设、`preload='auto'`；playbackService 按 §3.5c 时序实现 loadTrack/recordPlay/updateOutcome/seek/setVolume（音量持久化 debounce 500ms 调 settings:set）/toggleMute；playerStore 暴露 F4-2 状态集（currentTrack/duration/position(250ms 节流)/playing/volume/muted/playMode），position 更新用 `timeupdate` 事件节流，避免重渲染风暴。
  > 执行备注(2026-09-11)：三件套——`player/audioEngine.ts`（注入式工厂，缺省 new Audio()，模块顶层零副作用；buildAudioSrc 纯函数：wc-file:// + 整段 encodeURIComponent，评审 9 组对抗样例实证与 protocols.ts 整段 decode 严格可逆）、`player/playbackService.ts`（§3.5c 唯一落点：同曲守卫在最前连结算都不发；recordPlay fire-and-forget + 会话序号守卫防晚到回包污染；手动切歌/停止先 completed:0 结算（currentTime）次序有断言；暂停/恢复/seek 零 IPC grep 归类；volume 持久化 debounce 500ms 可测）、`stores/playerStore.ts`（F4-2 状态集，paused 不单列以计划为准；position 250ms 节流；createPlayerStore(deps) 工厂 + usePlayerStore 单例——T5.6 时 playerBridge 换实现零改调用方，T5.4 所需动作/状态集齐全）。**偏离留痕**：completed 传 boolean（IPC 契约优先于 §3.5c 文字 1/0）；muted 即时持久化（超计划取舍，离散动作无风暴风险，评审追认）；**快速切歌竞态备案**：recordPlay 未回包即切歌 → 旧会话 playedDuration 丢失（fire-and-forget 口径固有，playCount 正确，留痕可接受）。**评审（agent-2b370dfd）NEEDS_FIXES → 已修**：I1 loadContext([]) 空上下文不清场不结算（声音继续 + 旧会话被 ended 错记播完）→ 先 service.stop() 再清态 + 回归锚（旧会话 completed:0 + audio paused；无旧会话零计费用例同步收窄）。**用户裁定两项落地**：① repeat-one 下手动 next = 未切歌（service.next 特判：不结算、seek(0)+play、会话延续，ended 以原 historyId 落 completed:true——两条用例锚定；主流「next 跳下一首」语义需改 queue 冻结代码，如需另走裁定流程，留痕）；② T5.5 插队数据源 = service 层平行列表（见 T5.5 行备注）。变异：删手动结算路径 6 红 / 节流直通 1 红 / 特判禁用 1 红，均 hash 收口。**427 tests / typecheck 0 / e2e 11 passed**。
- [x] **T5.4 播放栏**（F4-3 全要素，视觉对照 components.html PlayerBar 区）：封面+标题+艺术家（点击标题跳专辑详情）、收藏按钮、上一首/播放暂停/下一首（播放暂停=32px 白圆反色按钮，`pause--on-accent.svg`/`play--on-accent.svg` 变体）、Shuffle 开关、Repeat 三态循环点击、进度条可拖动 seek（4px 槽 hover 增至 6px，已播填充用 `--text-secondary` 不用主色）、音量条+静音、队列按钮；播放中行均衡器动画（唯一持续动画）。全部控件 disabled 态可用（空队列时）。
  > 执行备注(2026-09-11)：F4-3 十二要素全落地（评审 agent-6560577e 逐要素核对 mockup.css/components.html 逐条一致）：PlayerBar.tsx 占位演进（92→~330 行）——封面+标题+艺术家（点击跳 /albums/:id）、收藏（乐观更新 + favorites:set + heart--accent 变体）、三传输钮（.player-button 32px 白圆反色 shell.css:465-479 与 mockup.css:1841-1852 逐条一致 + play/pause--on-accent 变体）、Shuffle aria-pressed+is-active、Repeat 三态 REPEAT_CYCLE off→all→one + 「1」角标、进度条 4→6px hover/已播填充 --text-secondary（design-plan.md:203 明文）/拖动松手 commit + role=slider 方向键、音量条+静音、队列按钮 onToggleQueue 回调（T5.5 接线点）、Equalizer 组件挂播放栏封面角标（reduced-motion 兜底）；空队列全部 disabled（含音量/静音/队列钮）。i18n 15 键全部预存在零新增。**设计稿事实澄清**：静音态图标 = volume-2 兼任是**设计稿有意取舍**（components.html PlayerBar 静音钮 data-lucide="volume-2" 仅 aria-label 变化，icons.md 同口径）——非资产遗漏，无需补 volume-x。**均衡器归属裁定依据（评审考证 design-plan 原文）**：§4.10 播放栏「封面带 3 柱均衡器角标」+ §4.11 QueuePanel「当前曲行 = accent 标题 + 均衡器」两处都要，曲目列表行在本版设计稿无对应原文；按「唯一持续动画=全应用唯一」字面口径 → 播放栏保留动画（本任务）、T5.5 QueuePanel 当前行用静态 --accent 指示、T5.6 曲目列表行静态 accent（不引入第二/第三无限动画）。**评审记账（不阻塞）**：①收藏 async 失败静默（fire-and-forget 契约，建议后续补 .catch(warn) 一行名实相符）；②拖动无 pointercancel 处理（桌面鼠标触发面极小）；③注释行号漂移两处；④css-contract strict 盲点新形态——媒体查询内部同名规则可顶替顶层裸规则（M2a 实证），守卫收紧为「顶层裸规则匹配」留待后续。变异 M1（REPEAT_CYCLE 错序）红 / M2b（全删 .equalizer-bar）红，hash 收口。**450 tests / typecheck 0 / e2e 11 passed**。
- [x] **T5.5 队列面板**：320px 右侧滑入（250ms transform，对照 components.html QueuePanel 区）；「正在播放 / 下一首播放（插队，`corner-down-right` 标识）/ 下次播放」三段结构同稿；0.1 只读（拖拽重排/删除属 F5-4 禁区，不渲染任何编辑 affordance）。  > 裁定备注(2026-09-11，用户拍板)：插队数据源 = **service/playerStore 层平行插队列表**（记录 playNext/enqueue 的 trackId，queue.ts 保持 §3.5b 逐字冻结不动），面板三段结构从平行数据渲染、与 queue 真实顺序由 service 保证一致；shuffle 下不依赖差集推断。
  > 执行备注(2026-09-11)：交付 `components/layout/QueuePanel.tsx`（三段结构逐字对照 components.html：正在播放/下一首播放/下次播放；当前曲行 music-2--accent **静态指示**——裁定②落实，无第二无限动画；插队行 corner-down-right 标识；**0.1 只读从严**：零编辑 affordance、不实现点击跳播，唯一交互=关闭按钮）+ playerStore 平行插队列表（userQueueIds + computeQueueView 三段推导 + advanceWithView 统一推进尾段；**「后插先播」序按 upNext 位置排序**——与真实播放序一致裁定落实）+ playerStore 新动作 playNext/enqueue（上下文外曲目补进 contextTracks 保证 resolve）+ AppShell queueOpen 态 + .queue-open 挂 .app-shell 祖先（设计稿后代选择器，与任务原文「挂面板本体」冲突从设计稿留痕）+ 关闭态 inert+aria-hidden（React 19 原生）。Escape/点外不关闭（常驻面板非模态，未定义留痕）；点击行不跳播（0.1 从严留痕）。**评审（agent-61ae58a3）APPROVED**：平行列表全场景推演一致（含 previous 回跳不回填=「插队意图=未播的用户意图」语义自洽、repeat-one restarted 不消费、三首插队交错「后插先播」排序实证、ghost id 丢弃+warn、重复插队双保险）；M1 删消费逻辑 2 红 / M2 退回登记序 1 红，hash 收口。**475 tests / typecheck 0 / e2e 11 passed**。评审备忘已落实：序号口径注释修正（设计意图=upNext 序不含正在播放，样本「02」否决全队列位置口径）+ rest 段重复行 key 去重；enqueue 重复曲目 rest 段如实呈现（0.1 无去重定义，接受现状留痕，T5.6 右键菜单 toast 时可顺带裁定）。
- [x] **T5.6 接通 T4.3/T4.4 占位**：TrackList 双击 = 所在页上下文整队（页面把当前视图的 TrackRow[] 与行号传 playerStore.playContext）；右键菜单「下一首播放/添加到队列」接 queue.playNext/enqueue；AlbumDetail 播放/随机播放按钮接通；不可播曲目双击 toast「M0.1 暂不支持此格式播放」。
  > 执行备注(2026-09-11)：接线矩阵全落地——playerBridge 换实现（playContext→loadContext / shuffleContext→loadContext(shuffleTracks(tracks),0)，消费方零改）；Songs 双击=当页视图整队（分页当页语义）、AlbumDetail/ArtistDetail 双击=全曲目、右键菜单两项接 playNext/enqueue（TrackRow 整对象）、AlbumDetail/ArtistDetail 播放/随机钮接通（随机=bridge 内单次洗牌）；不可播（missing/unplayable）双击/Enter/行内钮 → onUnplayableActivate → Toast（独立 toastStore 不混播放域、role="alert"+aria-live 常驻 stack、3s 自动消失 fake timers 锚定、覆盖重置计时、toast.css 逐字迁移 mockup.css:1944-1992 + css-contract 5 strict）；i18n player.unsupportedFormat 计划原文逐字。**特别留痕（混态接力）**：首次实现子代理因网络 502 中断但已部分写入工作区（变异残留 MUTANT + 详情页双重洗牌），重试子代理接手修复——评审（agent-6af44a77）全仓 grep 零残留、双洗牌单次化核实、残骸扫描无孤儿；修复变异复现恰好 2 例红与实现者声称一致。**评审 APPROVED**：接线矩阵逐格核实（index 语义无 virtuoso offset 错位）、M1 双击守卫删除 → 2 红 / M2 startIndex 固定 0 → 3 红（hash 收口）；Enter/行内钮不可播分支为合理对齐（主会话追认）；pageWiring.test.tsx（三页×双击/右键/toast 页面级接线测试，vi.mock virtuoso 行真实渲染）补记入产出清单（实现者自述漏报，评审揪出确认非残骸）；Albums 卡片播放钮保持占位（计划未列）、SearchResults 紧凑表不接（非 TrackList）、C1 播放行 accent 断言空窗移交 T5.7。**498 tests / typecheck 0 / e2e 11 passed**（主会话补跑 e2e 实证）。
- [x] **T5.7 E2E `playback-queue.spec.ts`**：fixtures 库 → 专辑页双击第 2 首 → 队列面板出现整专辑且高亮第 2 首 → `audio` 元素 playing → 进度推进 → 手动 next → 插队一首 → 断言下一首为插队曲 → Shuffle 点击后队列顺序变化且当前曲不变 → Repeat 三态切换 UI 正确 → 单曲内 pause/seek 后 playCount 不变（查 `library:getTrack` 断言 playCount=1，再加载一次 =2）。

  > 执行备注(2026-09-11)：交付 `tests/e2e/playback-queue.spec.ts`（5 用例，连跑 2 轮无 flaky）——逐条锚定计划步骤：①双击第 2 首→队列面板三段+高亮第 2 首+audio playing+进度推进（fixtures 音轨 ~1-3s 自动切歌，稳定断言前先 freeze 暂停——时序规避决策留痕）；②手动 next→右键插队→断言下一首为插队曲（面板插队段 corner-down-right + 消费后清空）；③Shuffle 起点改 nth(0) 双击（upNext 确定性锚定）+ 重复 reshuffle 16 次统计观测（碰撞概率 ~1.5e-5，规避单点 flaky）；④Repeat 三态 UI（aria-label/pressed/badge）；⑤playCount 双击=1 → pause/seek 零 IPC 不变 → **page.reload() 重置内存会话**再加载=2（同曲守卫语义下的合规再计费路径，实测成立）。**C1 断言空窗回收**（T4.3 备案）：tracklist.spec.ts:116 仅加指路注释；等价回收落 playback-queue（队列面板维度）+ 缺口闭合（见下）。**两处源码缺口闭合（主会话裁定，e2e 子代理上报）**：①audioEngine 缺省路径挂 DOM（幂等守卫，e2e 可观测性——此前 detached audio 无法 DOM 断言）+1 单测；②Songs/AlbumDetail/ArtistDetail 接 `playingTrackId`（新增 `usePlayingTrackId()` 选择器钩子——useSyncExternalStore 只订 id 变化，不随 position 节流重渲整表）→ 曲目表 `.track-row.is-playing` 出现，C1 双探针 warn→硬断言通过（队列面板 + 曲目表双维度）。**计划措辞偏差接受留痕**：队列「整专辑」实为「正在播放+upNext」（queue.upNext=order.slice(index+1)，冻结模型不含已播曲目）——断言改为贴合真实模型（queue-row=tracks.length-1 / upNext=slice(2)）。**498→502 tests / typecheck 0 / e2e 16 passed（11 基线 + 5 新增，主会话自证实跑）**。
...
- [x] **T6.0 Phase 6 前置小包**（Phase 5 收尾评审产出，2026-09-11 用户裁定先行）：① `ensureVolumeRestored()` 接线——App 挂载调用（音量/静音持久化只写不读的跨任务缝隙）+ 启动恢复单测；② **audio error 事件链路**——AudioEngine `error` 事件订阅（现零订阅）→ service 结算口径（**备案裁定：「loadTrack 即计」口径不回冲**——CONTEXT.md 计数语义锚在 loadTrack，error 时补 updateOutcome(completed:0, playedDuration:0) 结算会话，不新增递减 IPC；不自动跳下一首避免坏文件连跳风暴）→ UI 错误态（toast）+ playing=false → 单测 + e2e（损坏文件路径）；③ **QueuePanel 关闭焦点回落**（计划 903 行验收项）——关闭后焦点回触发按钮（PlayerBar 队列钮；onToggleQueue 传触发元素或 AppShell 持 ref）+ 单测 + 键盘走查留痕。

  > 执行备注(2026-09-11)：**①ensureVolumeRestored 接线**（AppShell 挂载 effect——App.tsx 为纯路由出口的落位取舍留痕；幂等守卫在 await 前置位，StrictMode 双调用 settings.get 恰一次有断言；恢复失败即置位不重试属既有取舍）。**②audio error 链路**（跨任务系统性缺口闭合）：playbackService.handleError（settleOutcome(0,false) + pause + `{type:'failed'}` 扩 AdvanceResult）；playerStore 订阅 audio.on('error')（**订阅点 service→store 偏离任务原文，评审接受**——与 ended 同形保持 store→service→audio 单向、service 仍可单测）→ playing:false + 保留 currentTrack/position + toast（store 直引 toastStore，与页面层先例一致无环）；i18n player.playFailed。**口径逐条落实**：仅一笔 updateOutcome(completed:0,playedDuration:0)、无 recordPlay 重发、不跳下一首、无递减 IPC、无会话不误报 toast、结算幂等。**e2e 可行性**：库内无损坏曲（08-broken.mp3 70 字节被扫描跳过——评审独立核实），但样本随 fixtures 拷入启用目录 → 构造合法 wc-file:// 注入触发解码失败（不绕过协议校验，200 为预期）→ 断言 toast/播放钮回退/当前曲不变/playCount=1；该用例豁免 console error 断言（故意注入失败源）但 pageerror 仍为空。**③QueuePanel 焦点回落**：PlayerBar onToggleQueue 签名改传触发元素 + AppShell queueTriggerRef + closeQueue 时 focus；全仓调用方仅 AppShell。**评审 APPROVED**：M1 删 error 结算 → 1 红；M2 删 trigger.focus → 1 红；**M3 附加**（volumeRestored 置位移到 await 后）→ settings.get 由 1 变 2、用例红——证明幂等断言真锚定守卫位置。**509 tests / typecheck 0 / e2e 17（两轮绿）**。**备案（评审确认项）**：error 后同曲重试不重建会话（playCount 已计 1，该次成功播放 completed 不落账——与「loadTrack 即计」口径自洽，接受）；shell-navigation 请求捕获类 flaky 为既有缓存竞态与该改动无因果（评审两轮未复现）。
- [x] **T6.1 收藏**：TrackList ♡/♥（`heart.svg`/`heart--accent.svg` 变体切换）、播放栏收藏按钮、Liked 页（绿色低透明渐变 Hero + `arrow-up-down` 排序下拉，5 键白名单，对照 Liked.html）；三处状态全局同步（同一 zustand 切片）。
  > 执行备注(2026-09-11)：**①术语整改（用户裁定「收藏」）**——docs/design/** 只读约束解除后执行：design-plan.md 4 处 + icons.md 1 处 + mockups 12 个文件（nav 链接/Liked 页全文案/toast）+ mockup.js 1 处 + Sidebar 注释 1 处，共 33 处（「喜欢的音乐」→「收藏」、「喜欢的歌曲」→「收藏的歌曲」）；**只改文案零视觉改动**（评审程序化验证：12 个 HTML 的 class 词元/标签序列/色值多重集全部零差异）；docs/design 内 0 残留（python 核验）。**术语范围裁定**：`docs/finale-analysis.md` 与 `docs/setting-up-plan.md` 属历史需求/计划正文**保留原文不改**（沿用 §3.4「历史原文 + 备注引用」口径）——产品面（设计稿/渲染层/i18n）已统一；主会话已补 §4.3「唯一渐变」表述对齐（获准两处之一）。**②共享收藏切片**：新建 `stores/favoritesStore.ts`（可测工厂 + 单例；FAVORITE_SORT_KEYS 5 键与 repo/ipc 三处同序；toggle 乐观+per-id 回滚+warn——顺手修 T5.4 评审记账的失败静默；ensureLoaded/refresh/isFavorite）；TrackList 加 `favoriteIds` prop（单点合成派生，**纯展示零 store import 纪律不破**）；loaded 门控防「空集合=全未收藏」假数据（未加载回退 track.favorite）；PlayerBar 改切片消费；AppShell 启动 ensureFavoritesLoaded；三页传参接线。**③Liked 页**：Hero 渐变 .hero--liked（**第二处获准渐变**，design-plan §4.6 明文）+ 排序下拉（arrow-up-down + 5 键白名单 → refresh 服务端排序）+ TrackList 复用 + 空态；进入即 refresh（主动偏离计划「首次 ensureLoaded」——保证列表→Liked 即时同步，评审接受）。**评审（agent-02beacdc）NEEDS_FIXES → 已修**：M2 无锚（TrackList 派生机制零测试保护）→ 新建 `TrackList.favorite.test.tsx` 3 例（切片覆盖 track.favorite 双向 + 省略回退），**M2 变异复验 2 红**；`empty.liked.hint` 旧术语残留（「收藏你喜欢的歌曲」）→ 改「点击曲目行的 ♥ 即可收藏」；M1（删回滚）2 红。备案：并发双失败回滚以最近快照为准（留痕）；Liked 首帧空态一闪（minor）；'全库 0 残留' 声称修正为「design 范围内 0 残留」。**534 tests / typecheck 0 / e2e 17 passed**。
- [x] **T6.2 歌单 CRUD**：Playlists 页（网格 + CollageCover + 虚线「新建歌单」卡 + 内联命名输入，对照 Playlists.html）；PlaylistDetail（双击标题内联重命名 `pencil`、删除=行内确认条（`--danger` 文字，3s 无操作收回）、添加曲目右键子菜单、移除曲目；对照 PlaylistDetail.html）。
- [x] **T6.3 拖拽重排**：PlaylistDetail 曲目表 HTML5 DnD（`grip-vertical` 手柄；dragstart 行 60% 透明、dragover 目标行上缘 2px `--accent` 插入线——mockup 已给视觉样本，本任务实现交互）→ 全量 `playlists:reorder`（repo 层事务重写，见 T1.4）。
- [x] **T6.4 CollageCover**：canvas 2×2 绘制前 4 首封面（不足 4 用占位色块）导出 dataURL，仅列表展示用（不落库，D-1）。
  > 执行备注(2026-09-11~12)：三件套合并实现（含前次 502 中断会话残留成果的接力修复——**playlistRepo.reorder 守卫"有注释无语句"真缺陷被单测揪出**，已补 `if (trackIds.length===0) return`）。**T6.2**：playlistsStore（可测工厂+单例；create 空名拒绝+creating 重入守卫/rename 乐观+回滚/remove 乐观/addTracks 非乐观取完整行+计数刷新/reorder 空数组拒绝+orderTracksByIds 乐观+失败回滚——空名分层=store 拒绝+页面回退「未命名歌单」双保险）；Playlists 页（网格+虚线新建卡+内联命名 Enter/Escape+卡片播放先 get 再 playContext——**trackCount=0 不渲染死按钮**）；PlaylistDetail（双击重命名 pencil+删除确认条 3s fake timers 锚定 role=alert+移除+跳回）；Sidebar 歌单列表+新建按钮接线（listLoaded 门控不渲染假歌单）；repo 两守卫（reorder 空数组 no-op 真语句+addTracks FK 友好错误「歌单或曲目不存在」+事务回滚无残留 COUNT=0）；**T6.3**：TrackList 加 draggable/onReorder props（缺省 false 非歌单页零影响+「其余页面不受影响」回归用例；重复曲目行键 id#index 防碰撞）+ TrackRowItem 四事件+手柄格+表头 sr-only；拖拽视觉 60% 透明/上缘 2px accent 插入线（mockup.css 为下缘——与「插到目标行之前」语义自洽的偏离留痕）；applyTrackOrder 向下拖插 to-1/向上拖插 to；失败回滚 prevDetail 快照。**T6.4**：collageCoverUtils 纯函数（resolveCollageTiles 恒 4 格不足补 null/collageCellRect 2×2 行优先奇数 floor/占位色 #242424）+ CollageCover 组件（canvas→dataURL→降级 DOM 拼贴三层；**不落库 D-1**）+ **CSP img-src 追加 data:**（风险评估：data: 图像不执行脚本无 XSS 向量，注入面仅应用自身 toDataURL 产物——评审认可）。**评审（agent-1aa9f16d）APPROVED**：M1 删 reorder 回滚 1 红 / M2 删确认条 3s 1 红（hash 收口）；e2e 两轮 19/19 无 flaky（新增 2 条：CRUD 往返 + reorder reload 持久化 F6-3 缩样）。**630 tests / typecheck 0 / e2e 19 passed**。**备忘**：①「添加到歌单」子菜单本轮仅 PlaylistDetail 接线（usePlaylistMenu 已就绪，Songs/详情页/Liked 接线随 T6.6 前置——T6.6 e2e 需在 Songs 右键添加）；②设计稿 6 列紧凑表→TrackList 完整 10 列+拖拽格（偏离留痕 PlaylistDetail.tsx:39-42）；③空歌单保留 Hero（空态下操作可达）；④插入线上缘 vs mockup 下缘偏离自洽留痕。
- [x] **T6.5 最近播放页**：`history:listRecent`；显示时间相对格式（i18n 内做 `formatRelative`）。
- [x] **T6.6 E2E `favorites-playlists.spec.ts`**：收藏一首（列表行）→ 播放栏同步为 ♥ → Liked 页出现且排序正确 → 取消同步；新建歌单 → 右键添加 3 首 → 拖拽第 1 首到第 3 位 → 重启应用（重新 launch 同一 WC_USER_DATA）顺序持久（F6-3）→ 删除歌单；播放两首（一首播两次）→ Recent 去重且只占一行、时间为最新（F7-2）。

  > 执行备注(2026-09-12)：**T6.5**：formatRelative 纯函数（i18n/formatRelative.ts——parsePlayedAt 显式 Date.UTC 解析 SQLite datetime('now') 秒精度、本地时区渲染、分段=刚刚/今天 HH:mm/昨天/M 月 D 日+跨年；**无独立「N 小时前」段**（同日≥1h 并入今天 HH:mm，任务书原文分段依据，留痕）+ 11 单测含 59min→61min 边界与 zh-CN.json 防漂移锁）+ Recent 页（轻量 6 列表非 TrackList——无播放时间列且行只读留痕；limit=100、行序=服务端 MAX(id) 去重最新在前、空态「开始播放」照稿；「最近 30 天」未实现留痕——repo 无时间窗参数副标题如实改条数上限）。**四页子菜单接线**（评审备忘立案）：Songs/AlbumDetail/ArtistDetail/Liked 传 {...usePlaylistMenu()}（照 PlaylistDetail 模式）+ 5 锚定用例（addTracks 实参级+toast 哨兵）。**T6.6**：favorites-playlists.spec 三段——收藏段（行 ♡→播放栏 aria 同步→Liked 首位=服务端 favorited_at 权威序→取消回 ♡）；歌单段（新建内联命名→右键加 3 首 waitForFunction 服务端落库→DataTransfer 拖拽 DOM 序+repo 双断言→**完整应用重启**（fixtures.ts launchWithUserData 同 WC_USER_DATA 二次 launch，T0 钩子 app.setPath('userData')）UI 复核顺序→行内确认条删除——F6-3 升级为完整重启实现）；Recent 段（真实双击播 A/B/A→2 行 A 最新在前+时间列「刚刚/N 分钟前」正则；freeze+playCount 轮询双保险防短音轨自动切歌）。**评审（agent-8a327f2f）APPROVED**：M1 昨天阈值破坏 1 红 / M2 行序反转 1 红（hash 收口）；UTC 解析+本地渲染一致性核实（读写两侧闭环）；launchWithUserData 顺序 launch 无 profile 锁风险（close 静默吞异常为低风险备忘）；重复添加 toast 口径=T6.2 既定（如实呈现，精细化另立）。**656 tests / typecheck 0 / e2e 22×2 轮零 flaky**。
**预期产出**：收藏/歌单/历史三块完整闭环。
...
- [x] **T7.1 Settings 页骨架**（F8-1）：分区锚点导航：General / Library / Playback / About；修改即时生效 + 持久化（settings:set 后确认回读）；控件（40×22 开关、`--bg-input` 进度槽）对照 Settings.html 施工。
- [x] **T7.2 General**：语言选择（当前仅 zh-CN，下拉含占位提示「English · 0.5 提供」）；说明文案引 §8 已知限制。
  > 执行备注(2026-09-12)：Settings 页一体成型（224 行 + settings.css mockup.css:1425-1620 收口）。**T7.1**：四分区锚点导航（**button+scrollIntoView 替代 mockup `<a href="#x">`——HashRouter 下页内锚改写路由 hash 成死链的偏离留痕**，e2e 导航矩阵保持绿）+ aria-current；40×22 开关与 --bg-input 进度槽基建就位（本批不渲染不登记，T7.3 补登留痕）。**T7.2**：语言下拉 zh-CN 选中 + **English disabled「English · 0.5 提供」**（grill ③）；mockup「上次页面」开关无 settings 键支撑不渲染（四键无此项不扩键）。**T7.4**：音量默认值 pill 百分比**只读回显**（grill ②）；Metadata/Cache 零占位仅 About roadmap 声明（YAGNI）。**T7.5**：**新增最小 app:getVersion 通道**（shared 契约 + main getAppVersion? 注入默认惰性 electron.app.getVersion() + preload 新 `app` 命名空间 + preload-contract 9 键锁）+ About 版本 1.0.0 如实显示 + §8 八条逐字（finale-analysis:306-313 对照零偏差）+ 技术栈致谢 + roadmap。**T7.1 承接（QueuePanel selector 化）**：新增 useQueueView()（照 usePlayingTrackId 先例）——position tick 仅 set({position}) 不触碰 queueView 引用（Object.is 判定不重渲，**关键核查点经评审读 set 逻辑证实非假优化**）；Profiler 计数测试（tick 1 vs usePlayer 对照 6）。**确认回读形态**：settings:set 返回合并后完整 Settings 作确认（handler 本身产出合并结果，二次 get 纯浪费——评审认可）。**不建 settingsStore**（写低频/四键/无第二消费方）。**评审（agent-fc1ca2cd）APPROVED**：M1 版本 handler 固定值 1 红 / M2 English disabled 移除 1 红（hash 收口）；§4.3-2 渐变表述对齐 + Settings.html 核对；并行编辑竞态 5 处丢失经首次 e2e 红暴露后全仓审计补齐（main/ipc 注释合并残留已由主会话拆回两行）。**682 tests / typecheck 0 / e2e 22 passed**。
- [x] **T7.3 Library**（F8-2 / F1-1 / F1-5）：目录列表（`folder` 图标 + 真实 Windows 路径写法 + 启用开关 + `trash-2` 删除）；删除 = 行内确认条（对照 Settings.html 样本，禁系统弹窗）；添加按钮 → `library:addFolder`（无 path 参数走系统对话框）；「启动时自动扫描」开关绑定 settings.autoScanOnStartup；「全量重扫」（`refresh-cw`）触发 `library:rescanAll` + 扫描进度条（订阅 scan:progress，显示 done/total 与阶段文案，同 Settings.html 进度样本）。
  > 执行备注(2026-09-12)：**任务书事实修正**——library:listFolders 通道原不存在，本批 additive 补建五处（常量/payload void/return FolderRow[]/preload/handler + preload-contract 键锁 + tsconfig include folderRepo）。**grill ①落实（markMissing 立即语义）**：handler 侧 markTracksMissingUnderFolder——listIdentityAll 内存前缀过滤（小写+反斜杠转正斜杠对齐 scanService 口径+尾补斜杠防 d:\MusicX 误伤，评审逐边界核实含 UNC/混合分隔符/相对路径安全侧失败）→ setStatus missing，SQL 零改动；removeFolder 先取路径→移除→onFoldersChanged→标 missing；setFolderEnabled 禁用→立即 missing、**启用→不恢复**（等下次扫描 C1 复活保 UUID，三处留痕）。**评审关键核查**：tracks 表**无 folder_id 列**（file_path UNIQUE 软关联）——FK CASCADE 顾虑不成立，「行保留」裁定天然安全；folderRepo.list() 无 enabled 过滤（禁用目录 UI 可见可操作）。**T7.3 UI**：目录列表/行内确认条 3s/添加按钮无参对话框（取消静默）/重扫防重入（UI 禁用+scanService inFlight 双保险）/自动扫描开关确认回读/进度条独立订阅卸载退订（scan:progress 经 webContents.send 广播无双订阅冲突）；scan:progress 'cover' 相位永不可达（T2.3 遗留，i18n 备位 done 同帧收起故不展示——备案）。**承接项（folderRepo.normalizePath 加固）**：空串/纯空白 throw、盘符根补分隔符（消除驱动器当前目录语义偏移）、UNC 主机段小写归一；**数据迁移备案**：旧规则存量行不自动迁移（与新行并存 UNIQUE 不冲突，手动删除即可）。**评审（agent-c44db899）APPROVED**：M1 setStatus 短路 3 红 / M2 空串 throw 失效 1 红（hash 收口）。**705 tests / typecheck 0 / e2e 22 passed**。
  > **收尾评审 Critical 修复（2026-09-12，收尾评审 agent-ba927cb9 NEEDS_FIXES → 已修）**：**UI 添加目录后未触发扫描**——计划 825 行 V1.7 明文「Phase 7 UI 接线时必须补扫描调用」+ README「UI 不得省略」，但 handleAddFolder 只刷新列表（新目录文件静默不入库）；单测固化无 scan 现状 + e2e 经 page.evaluate 直连恰好绕开 UI 按钮路径 = 任务级评审双漏、阶段级收尾才暴露的典型跨任务缺口。**修复**：handleAddFolder 成功分支补 `await api.library.scan()`（fire-and-forget，进度经 scan:progress 呈现；用户取消不触发）+ Settings.test 补 scanCalls 断言（harness 增 scan 桩）。修复后 **705 tests / typecheck 0 / e2e 26 passed**。
- [x] **T7.4 Playback**：音量默认值回显（实际音量控制在播放栏，此处只做说明与默认值）；留待 0.5 的 Metadata/Cache 分区不渲染占位（YAGNI，仅 About 里声明规划）。
- [x] **T7.5 About**：版本号（读 package.json version）、已知限制八条（需求 §8 逐条对应文案，走 i18n）、技术栈致谢（Electron/Chromium/SQLite/music-metadata/sharp/React）。
- [x] **T7.6 E2E `settings-folders.spec.ts`**：添加目录（直传 path）→ 重启仍在（F1-1 前半）→ 禁用 → rescanAll → 断言该目录文件 status=missing 或未被新扫（以 last_scan_at/解析计数佐证）→ 重新启用 → 恢复（F1-1 后半）；关掉 autoScanOnStartup → 重启不产生扫描事件。
  ...
  > 执行备注(2026-09-12)：交付 `tests/e2e/settings-folders.spec.ts`（4 用例，两轮全绿零 flaky）——a) 直传 path 加目录（UI 按钮走 showOpenDialogSync 不可自动化，经 page.evaluate 调 window.api.library.addFolder(path) 同 handler 直连——任务书已裁定合法）+ **launchWithUserData 双 launch 重启仍在**（F1-1 前半）；b) UI 开关禁用→**立即 missing**（grill ①裁定口径，比计划原文更强）→ rescanAll 仍 missing（回归原文）→ 重新启用→仍 missing（裁定：启用不恢复）→ rescanAll→全 available 且 **track id 集合与禁用前一致**（C1 保 UUID，F1-1 后半）；c) trash-2 行内确认条删除→曲目行保留全 missing+**收藏关系保留**（favorites.list 复核）→重启后目录空态；d) autoScanOnStartup 关→重启→**DB 级主证**（探针文件 09-autoscan-probe 未入库=启动扫描未发生，文件名与库内均不同 adopt 三键必 miss）+ 阳性对照（手动 rescanAll 同文件正常入库）——startupScan 在 app ready 触发早于可装监听时点，「未观察到事件≠未发生」的取证形态留痕 spec 文件头②。**源码观察上报（备案）**：`library_folders.last_scan_at` 列全链路无写入点（0001_init 建列无维护）——计划原文「以 last_scan_at 佐证」实际不可用，改用探针+解析计数佐证，不影响结论。**705 tests / typecheck 0 / e2e 26×2 轮全绿**。**预期产出**：设置页完整，目录管理满足 F1-1 全部验收。
...
- [x] **T8.1 图标与绿色产物**：`resources/icon.ico`（多尺寸 16/32/48/256，可用 ImageGen 或手工绘制后转换）；electron-builder.yml 按 §3.8 定稿；根目录运行 `build.bat` → 产出 `build\` 绿色目录（`Windy Concert.exe` + 依赖）；双击 `build\Windy Concert.exe` 冒烟（窗口起、扫描/播放可用）；`build.bat zip` 生成 `Windy-Concert-0.1.0-win64.zip`。
  > 执行备注(2026-09-12): 外部图标采用 SVG Repo `Vinyl Record` 528786（CC0，来源与变换记录见 `resources/icon-source.md`），保留唱片/声波几何并以 Windy Concert 深色+绿色 token 重绘；`resources/icon.png` 与 `resources/icon.ico` 由 `scripts/generate-app-icon.mjs` 生成，ICO 实测 16/32/48/256 四项。electron-builder.yml 按 §3.8 定稿为 `cn.windyconcert.app` / `Windy Concert` / `release` / `out/**` / locales+icon extraResources / `**/*.node` 解包 / NSIS 后置；package/package-lock 版本统一 `0.1.0`；`npm test` 706、typecheck 0、build exit 0、zip exit 0，产物与 native `.node` 内容经独立评审核实，启动/播放冒烟与安装器留待 M8。
- [ ] **T8.2 性能实测**（`scripts/gen-sample-library.mjs --count 30000`，磁盘预留 ≥2GB，测完删除）：按 §6.2 表逐项测量并记录结果到 `docs/perf-0.1.md`（新建，允许）：首扫（scan.log elapsedMs）、启动增量（二次启动 scan.log）、启动到可交互（手工秒表×3 取中位，从双击图标到 Songs 列表可滚动）、搜索（控制台打点×10 取中位，含中文 2 字与英文）、内存（任务管理器空库）、切歌起音（手工×10 取中位）、发布 zip 体积（build.bat zip 后看文件属性）。任一不达标 → 停止，走 §7 对应条目，不得带病发布。
  > 执行备注(2026-09-13): 自动门已补齐并复核（首扫 `scan.log elapsedMs=96252`、启动增量 `6319`、搜索中位 `0.5ms`、zip `157722528B`/160 项）；控制器串行补充了 packaged-flow M1/M2/M3/M4/M5/M8 辅助证据与三次启动代理 `869/724/736ms`（中位 `736ms`）。严格人工双击、拖拽、听音、快速滚动、输出设备切换仍未执行，因此 T8.2 checkbox 保持未勾选，M1-M8 人工行保持 `NOT MEASURED`。
