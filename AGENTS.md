...
- 2026-09-12：**Phase 7（设置页与 Library 管理，T7.1~T7.6）已完成；收尾评审 NEEDS_FIXES（1 Critical）→ 已修复达 PASS 线**。交付：Settings 页四分区（锚点导航 button+scrollIntoView——HashRouter 死链偏离留痕；English disabled/Playback 只读/About 最小——grill 四裁定落实）+ app:getVersion 通道 + QueuePanel selector 化承接 + **T7.3 Library 管理闭环**（listFolders 通道补建/markMissing 立即语义——tracks 无 folder_id 路径软关联评审核实/folderRepo 边界加固）+ T7.6 e2e 4 用例（F1-1 前后半+autoScan 探针取证 DB 级主证）。**收尾评审 Critical（已修）**：UI 添加目录后未触发扫描（V1.7 明文「必须」+ 单测固化无 scan + e2e 直连绕行双漏）→ handleAddFolder 补 await scan() + scanCalls 断言。**705 unit tests / typecheck 0 / e2e 26 passed**。**Phase 8 前置清单已收录 README**（electron-builder 定稿+icon.ico 缺失/last_scan_at 死列裁定/perf-0.1.md/M4 人工脚本/人工复核留痕等 7 项）。下一步等用户指令进入 Phase 8（打包与性能验收，M0.1 最后一阶段）。
