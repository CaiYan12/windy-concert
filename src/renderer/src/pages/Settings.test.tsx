 ...
 *   · Library：本批仅区头 + 占位文案（T7.3 填充本体，留痕）。
 *
 * api 桩：browseFixtures 的 apiStub 为 no-op Proxy，逐键覆写 settings / app 两命名空间。
 */
...
/** 覆写 apiStub 的 settings / app 命名空间（Recent.test 的 installHistoryApi 同款手法）。 */
...
describe('Library 分区（T7.1 最小占位）', () => {
  it('本批仅区头 + 占位文案；不渲染目录列表/扫描控件', async () => {
    const { container, unmount } = await mountSettings()
    const section = container.querySelector('section#library')!
    expect(section.textContent).toContain('«settings.library.placeholder»')
    expect(section.querySelectorAll('.folder-list, .folder-row, .switch, .progress-track')).toHaveLength(0)
    unmount()
  })
})
